...
//! [`typeship::ir::Decl`] holding ts-rs's finished `export …` line verbatim
...
//! [`Bridge`]: typeship::Bridge
//! [`DeclBody::Raw`]: typeship::ir::DeclBody::Raw

use ts_rs::{Config, TS};
use typeship::ir::Decl;
...
mod tests {
    ...
    fn struct_lowers_to_export_with_camelcase_fields() {
        ...
        // ts-rs applied the camelCase rename; typeship did not re-mangle it.
    ...
    fn data_carrying_enum_lowers_to_discriminated_union() {
        ...
        // typeship wraps that verbatim; we only assert it stays a closed union.
    ...
    fn bridge_assembles_ts_rs_decls_with_commands() {
        ...
        // Header + transport import from typeship, type bodies from ts-rs.
        assert!(ts
            .contents
